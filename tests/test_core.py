...
from codex_accounts.core import AccountManager, strip_account_name, validate_account_name
from codex_accounts.errors import CodexAccountsError
from codex_accounts.platforms import SystemPlatform
...
class TestAccountNames:
    def test_strip_account_name_accepts_paths_and_prefixed_names(self) -> None:
        assert strip_account_name("/Users/example/.codex-work") == "work"
        assert strip_account_name("C:\\Users\\example\\.codex-personal") == "personal"
        assert strip_account_name("team.dev") == "team.dev"

    @pytest.mark.parametrize("name", ["work", "personal_1", "team.dev", "a-b"])
    def test_validate_account_name_accepts_safe_names(self, name: str) -> None:
        validate_account_name(name)

    @pytest.mark.parametrize("name", ["", ".", "..", "bad/name", "bad name", "中文"])
    def test_validate_account_name_rejects_unsafe_names(self, name: str) -> None:
        with pytest.raises(CodexAccountsError):
            validate_account_name(name)
...
class TestAccountManager:
    def test_create_switch_and_show_current(self, tmp_path: Path) -> None:
        ...
        manager.create_account("work", [])
        manager.switch_account("work", ["--no-stop", "--no-start"], ["switch", "work"])
        manager.show_current()

        output = stdout.getvalue()
        assert "Created account directory" in output
        assert "Switched to: work" in output
        assert "work ->" in output
        assert manager.current_target().kind == "target"

    def test_list_accounts_marks_active_account(self, tmp_path: Path) -> None:
        manager, stdout, _ = make_manager(tmp_path)

        manager.create_account("work", [])
        manager.create_account("personal", [])
        manager.switch_account("personal", ["--no-stop", "--no-start"], ["switch", "personal"])
        stdout.seek(0)
        stdout.truncate(0)
    ...
    def test_switch_refuses_to_replace_real_directory(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(CodexAccountsError, match="not a symlink"):
            manager.switch_account("work", ["--no-stop", "--no-start"], ["switch", "work"])
    ...
    def test_migrate_current_moves_real_directory_and_links_it(self, tmp_path: Path) -> None:
        ...
        manager.create_account("personal", ["--migrate-current"])

        target = manager.account_dir("personal")
        assert (target / "config.toml").read_text(encoding="utf-8") == "token = 'test'\n"
        assert manager.current_target().kind == "target"
        assert "Migrated current account" in stdout.getvalue()
    ...
    def test_migrate_current_refuses_when_app_is_running(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(CodexAccountsError, match="is running"):
            manager.create_account("personal", ["--migrate-current"])
    ...
    def test_default_switch_skips_app_control_on_non_macos_platforms(self, tmp_path: Path) -> None:
        ...
        manager.switch_account("work", [], ["switch", "work"])
    ...
    def test_switch_from_codex_terminal_without_delegation_is_blocked(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(CodexAccountsError, match="built-in Codex terminal"):
            manager.switch_account("work", ["--no-stop", "--no-start"], ["switch", "work"])
    ...
    def test_switch_from_codex_terminal_delegates_when_available(self, tmp_path: Path) -> None:
        ...
        manager.switch_account("work", ["--no-stop"], ["switch", "work", "--no-stop"])

        assert platform.delegate_calls == [("switch accounts", ["switch", "work", "--no-stop"])]
